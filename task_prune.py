...
def capture_input_to_wdown(module, input, output):
    print(f'compressing mlp layer of {module._name} th decoder layer')
...
hooks = []
for name, module in model.model.layers.named_children():
    module.mlp._name = name
    hook = module.mlp.register_forward_hook(capture_input_to_wdown)
    hooks.append(hook)

logging.info("Pruning has started")
# Run a forward pass
with torch.no_grad():
    _ = model(**data)

logging.info("Pruning has been completed")
